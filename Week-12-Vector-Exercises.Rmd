...
```{r}
sw_people %>% head(3) #prints the information associated with first three characters
sw_people$`Luke Skywalker` #prints out information associated with just Luke Skywalker
sw_people[[2]] #prints out information associated with C-3PO
```
```{r}
pie <- list(x = c(1,2,3), y = "two", z = FALSE)
pie$x
```
...
```{r}
a_list["num"] #give is the list information but still attach it to the list

mean(a_list["num"])
```

```{r}
a_list[["num"]] #ives jsut the information in the vectore
